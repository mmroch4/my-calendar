...
const admittedRadixColorsEntries = radixColorsEntries.filter(([colorKey, colorValue]) => {
  return !colorKey.endsWith('A');
});
...
const formattedRadixColorsKeys = Object.entries(admittedRadixColors).map(
  ([colorKey, colorValues]) => {
    const formattedColorValuesKey = Object.entries(colorValues).map(([key, entry]) => {
